...
class AdvancedHypothesisParser:
            ...
            for category, terms in self.stats_vocabulary.items():
                if lemma in terms:
                    entities["statistical_terms"].append(
                        {"term": lemma, "category": category, "pos": token.pos_}
                    )

            # Extract comparison words
            if token.pos_ in ["ADJ", "ADV"] and lemma in [
                "more",
                "less",
                "higher",
                "lower",
    ...
    def _extract_comparison_advanced(
        ...
        for comp_type, config in self.comparison_patterns.items():
            ...
            for pattern in config["patterns"]:
                ...
                if match:
                    ...
                    groups = match.groups()
                    if len(groups) >= 2:
                        comparison_scores[comp_type] = (
                            score,
                            (groups[0].strip(), groups[1].strip()),
                        )
                    else:
    ...
    def _infer_columns_advanced(
        self, text: str, variables: List[str], entities: Dict, data=None
    ...
    def _refine_test_type(
        ...
        if group_dtype in ["object", "category", "bool"] and value_dtype in [
            "int64",
            "float64",
            "int32",
            "float32",
        ...
        elif group_dtype in ["object", "category", "bool"] and value_dtype in [
            "object",
            "category",
            "bool",
        ]:
            ...
            "int32",
            "float32",
        ] and value_dtype in ["int64", "float64", "int32", "float32"]:
            return "correlation"
...
class SimpleHypothesisParser:
    ...
    def parse(self, hypothesis_text: str, data=None) -> ParsedHypothesis:
        ...
        if re.search(r"\b(?:equal to|different from|greater than|less than)\b.*\d", text):
            ...
            r"\b(?:multiple groups|across groups|more than two|several groups|\banova\b)\b",
            text,
        ):
            test_type = "anova"
...
def create_parser(
    advanced: bool = True,
) -> Union[AdvancedHypothesisParser, SimpleHypothesisParser]:
