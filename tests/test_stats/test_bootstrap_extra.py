...
from hypotestx.math.statistics import mean as _mean, median as _median
...
class TestBootstrapTwoSampleCI:
    ...
    def test_true_diff_inside_ci(self):
        lo, hi, _ = bootstrap_two_sample_ci(GROUP_HIGH, GROUP_LOW, ci=0.95, n_resamples=2000, seed=7)
    ...
    def test_custom_diff_fn(self):
        ...
        lo, hi, boot = bootstrap_two_sample_ci(GROUP_HIGH, GROUP_LOW,
                                               diff_fn=diff_median,
...
class TestBootstrapTest:
    ...
    def test_alternative_greater(self):
        high_data = [10.0] * 20
        p, _, _ = bootstrap_test(high_data, _mean, null_value=5.0,
                                 alternative="greater", n_resamples=500, seed=0)
        assert p < 0.05

    def test_alternative_less(self):
        low_data = [1.0] * 20
        p, _, _ = bootstrap_test(low_data, _mean, null_value=5.0,
                                 alternative="less", n_resamples=500, seed=0)
        assert p < 0.05

    def test_invalid_alternative_raises(self):
        with pytest.raises(ValueError):
            bootstrap_test(DATA, _mean, null_value=3.0,
                           alternative="invalid", n_resamples=100)
...
class TestPermutationTest:
    ...
    def test_alternative_greater(self):
        p, _, _ = permutation_test(GROUP_HIGH, GROUP_LOW,
                                   alternative="greater", n_resamples=500, seed=0)
        assert p < 0.05

    def test_alternative_less(self):
        p, _, _ = permutation_test(GROUP_LOW, GROUP_HIGH,
                                   alternative="less", n_resamples=500, seed=0)
        assert p < 0.05
    ...
    def test_custom_statistic_fn(self):
        def diff_median(a, b):
            return _median(a) - _median(b)
        p, obs, perm = permutation_test(GROUP_HIGH, GROUP_LOW,
                                        statistic_fn=diff_median,
                                        n_resamples=500, seed=1)
        assert isinstance(p, float)
...
class TestBootstrapCIBca:
    ...
    def test_bca_covers_mean(self):
        lo, hi, _ = bootstrap_ci(DATA, _mean, ci=0.95, n_resamples=2000,
                                 method="bca", seed=99)
